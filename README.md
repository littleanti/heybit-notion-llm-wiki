...
| 항목 | 상태 | 설명 |
|---|---|---|
| **실 Notion 워크스페이스 동작** | **미실측** | 모든 API 호출은 공식 문서(2026-09-09 확인, [TRD 3.1](./docs/TRD.md#31-notion-api--문서로-확인한-사실-2026-09-09-n15n16-은-2026-09-10))의 JSON 형태를 흉내 낸 mock 으로만 검증했다. 실제 응답이 다르면 fixture 와 코드를 함께 고쳐야 한다 |
...
| **작성·수정 스킬의 실 Notion 동작** | 미실측 | `POST /v1/pages` 가 속성과 본문을 한 번에 받는지, `PATCH /v1/pages` 의 속성 부분 갱신, `GET /v1/users` 의 이름 해석은 **문서 확인 + mock 검증**이다. 동명이인·게스트·비활성 사용자에서 어떻게 되는지는 실측이 필요하다 |
