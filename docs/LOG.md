...
**위키 합성 실측 (`ingest --full`)**: 스킬 절차 문서만 준 작업자 2개가 서비스별로 수행해 **위키 30페이지**를 만들었다
(루틴핏 15 = 개요1·다이제스트7·토픽6·충돌1, 머니노트 15 = 개요1·다이제스트6·토픽7·충돌1). `lint` **오류 0**, 색인 103줄(raw 34 · wiki 30).
- 두 작업자가 **독립적으로 같은 결함**을 잡았다 → 아래 세 번째 결함.
- 실 워크스페이스의 구조적 한계도 드러났다: 시딩으로 만든 페이지들은 **`last_edited_time` 이 전부 같아** overview 의
  "지금 알아야 할 것" 을 미러 시각으로 정렬할 수 없다. 두 작업자 모두 각 원본의 `## 변경 이력` 마지막 날짜를 기준으로 삼고
  그 근거를 페이지에 적었다. wiki-schema 에 **정렬 기준의 대체값**을 명시할 일이다(미확정).
- 절차 문서의 모호점도 다시 나왔다: 다이제스트·개요·충돌 페이지의 **frontmatter `title` 규약이 없다**(게시기는 `displayTitle` 로
  덮어쓰므로 실제 게시에는 영향이 없었다), `meta 없음` 원본을 담을 템플릿 절이 없다, 상태 불일치도 "충돌" 로 볼지.

**세 번째 결함 (`[fix]` 완료)**: `draft-submit` 이 미러를 쓸 때 `writeRawMirror` 에 **설정 객체 대신 slug 문자열**을 넘겨
`service`·`service_name`·`category`·`category_name` 이 전부 `null` 인 raw 파일을 만들었다. `lint` 가 `E meta-invalid` 를 냈다.
- T21 이 이 필드를 검사하지 않아 놓쳤다. 파일 경로에는 slug 가 쓰여 경로는 정상이었던 것이 발견을 늦췄다.
- 수정: 객체를 넘기고, 함수 진입부에서 문자열을 받으면 즉시 실패하게 했다. T21·T22 에 네 필드 검사를 추가했다.
- `sync --full` 로 미러를 복구했다 (raw 34, 범위 필드 정상).

**게시 실측 (`publish`)**: dry-run 계획 34건(색인·이력·서비스 컨테이너 2·잎 30)을 확인한 뒤 `--apply`.
- **34페이지 생성 성공.** 2회차 `--apply` 는 **생성 0 · 교체 0 · 변경 없음 32** — 멱등성이 실 API 에서 확인됐다(A11).
- 게시된 페이지를 다시 읽어 검증: 마커 콜아웃 그대로, `⚠ 충돌 있음` 첫 줄, 그리고 **링크 30개 전부 실제 Notion 원문 URL 로 해석**되고
  미해석 상대 경로 **0건**. 위키 → 원문 왕복이 실환경에서 성립한다.
- 게시 루트: `LLM Wiki (테스트)` 아래 서비스 컨테이너 2개.

**해소된 미확정** (P0 부터 남아 있던 것들)
| 항목 | 결과 |
|---|---|
| 실 Notion 워크스페이스 동작 | **해소** — 발견·미러·색인·lint·작성·게시 전 경로가 실 API 에서 동작 |
| enhanced markdown 렌더 | **해소** — 콜아웃·표·중첩 목록·인용·코드 펜스가 그대로 왕복 |
| `POST /v1/pages` 가 속성+본문을 함께 받는지 | **해소** — 속성 12개 + 본문을 한 호출로 생성 |
| 담당자 이름 → 사용자 id 해석 | **해소** — `GET /v1/users` 로 해석, 왕복 확인 |
| 증분 동기화 호출 절약(A2) | **해소** — 2회차 본문 호출 0건 |
| 게시 멱등성(A11) | **해소** — 2회차 쓰기 0건 |

**남은 미확정**: `POST /search` 인덱스 지연(이번엔 시딩 직후에도 전부 발견됐다), 본문 교체가 페이지 코멘트를 유지하는지,
동명이인·게스트 사용자에서의 담당자 해석, 수백 페이지 규모의 색인 토큰 수, 실무자 사용성.

**저장소에는 실 워크스페이스 산출물을 넣지 않았다.** 샘플 `raw/`·`wiki/` 는 fixture 기반 골든이고 테스트가 그것에 걸려 있다.
실 워크스페이스의 페이지 id 를 공개 저장소에 넣을 이유도 없다. 실 테스트는 별도 작업 폴더에서 했다.
